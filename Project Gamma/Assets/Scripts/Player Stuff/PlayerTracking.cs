...
public class PlayerTracking : MonoBehaviour {
	...
	void Awake () {
		currentDis = -distance;
		cameraController = camPivot.GetComponent<CameraController> ();
	}

	// Update is called once per frame
	void FixedUpdate () {
		if (cameraController.activePlayer == player.gameObject) {
	...
	void MoveCam () {
		...
		if (Physics.Linecast (player.position, transform.position, out hit, 1 << 8) && hit.distance < distance) { // Casts a line from the camera pivot to the camera. The if statement runs if it hits something
			currentDis = -Mathf.Clamp (hit.distance - 0.6f, 1f, distance);
			...
			currentDis = -Mathf.Clamp (-currentDis + 0.1f, 1f, distance);
		}
